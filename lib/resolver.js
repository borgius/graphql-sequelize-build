...
function resolverFactory(target, options) {
  ...
  resolver = function (source, args, context, info) {
    ...
    if (options.filterAttributes) {
      findOptions.attributes = Object.keys(fields).map(key => fields[key].key || key).filter(key => targetAttributes.includes(key));
    ...
    return (0, _generateIncludes2.default)(simpleAST, type, context, options).then(includeResult => {
    ...
    }).then(result => options.after(result, args, context, _extends({}, info, {
      ast: simpleAST,
      type: type,
      source: source
    })));
