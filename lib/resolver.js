...
function resolverFactory(target, options = {}) {
  if (options.useDataLoader !== false) (0, _dataloaderSequelize2.default)(target);
  ...
  resolver = async (source, args, context, info) => {
    ...
    return Promise.resolve(options.before(findOptions, args, context, info)).then(findOptions => {
      ...
      if (Array.isArray(findOptions.include)) {
        ...
        findOptions.include.forEach(fixIncludeOffset); //build: fix include offset sequelize bug
    ...
    }).then(result => options.after(result, args, context, info));
  };
