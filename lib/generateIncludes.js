...
function generateIncludes(simpleAST, type, context, options) {
  ...
  return Promise.all(Object.keys(simpleAST.fields).map(function (key) {
    ...
    if (includeResolver.$passthrough) {
      return generateIncludes(fieldAST, fieldType, context, options).then(function (dummyResult) {
    ...
    if (association) {
      ...
      if (options.filterAttributes) {
        includeOptions.attributes = (includeOptions.attributes || []).concat(Object.keys(fieldAST.fields).map(key => fieldAST.fields[key].key || key)).filter(inList.bind(null, allowedAttributes));
      } else {
      ...
      return Promise.resolve().then(function () {
        ...
        if (include && (!includeOptions.limit || separate)) {
          ...
          return generateIncludes(fieldAST, fieldType, context, includeResolver.$options).then(function (nestedResult) {
